...
async def test_broadcast_venus_data_success(mock_influx_client, sample_data):
    ...
    task = asyncio.create_task(broadcast_venus_data(queue, mock_influx_client))
...
async def test_broadcast_venus_data_influxdb_error(
    mock_influx_client, sample_data, caplog
):
